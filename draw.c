...
int wireframe = 0;
/*
  0 is no wireframe (scanline converting)
  1 is wireframe (no scanline converting)
*/

double view_vector[3] = {0.0, 0.0, -1.0}; // Where are we??
double light_vector[3] = {0.0, 0.0, -1.0}; // Where is the light coming from??
...
void draw_polygons( struct matrix *polygons, screen s, color c ) {
  double **m = polygons -> m;  // Because I am lazy

  int i;

  double **polygon_normal_vectors = (double **) malloc(sizeof(double *) * polygons -> lascol / 3);
  double **vertex_normal_vectors = (double **) malloc(sizeof(double *) * polygons -> lascol);
  ...
  for( i=0; i < polygons->lastcol-2; i+=3 ) {
	...
	double x_bottom, y_bottom, z_bottom, x_middle, y_middle, z_middle, x_top, y_top, z_top;

	double vertex_index_top, vertex_index_middle, vertex_index_bottom; // If vertex 0 is the top, then vertex_index_top should be 0
	
	// SCANLINE CONVERSION

	/* THREE CASES:
	   1) If the vertices have not common y value
	   2) If the first two vertices are on the bottom (same y coord)
	   3) If the last two vertices are on the top (same y coord)
	*/

	// This part is annoying, this is going through and making sure that x_bottom, y_bottom, and z_bottom, etc are all correct
	if (y0 != y1 && y0 != y2 && y1 != y2) {
	  scanline_case = 1;
	}
	else if (y0 == y1 && y0 < y2) { // POSSIBLE ERROR
	  scanline_case = 2;
	}
	else {
	  scanline_case = 3;
	...
	if (scanline_case == 1) {
	  ...
	  if (y0 < y1 && y0 < y2) {
	    ...
	    y_bottom = y0;
	    z_bottom = z0;
	    vertex_index_bottom = 0;
	    if (y1 < y2) {
	      x_middle = x1;
	      y_middle = y1;
	      z_middle = z1;
	      vertex_index_middle = 1;
	    ...
	    y_bottom = y1;
	    z_bottom = z1;
	    vertex_index_bottom = 1;
	    if (y0 < y2) {
	      x_middle = x0;
	      y_middle = y0;
	      z_middle = z0;
	      vertex_index_middle = 0;
	...
	} // closes case 1 for scanline

      } // closes the entire scanline section
      /*
